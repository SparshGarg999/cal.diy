...
Since Cal.diy is a community-maintained fork, pre-built Docker images are not published to Docker Hub. The Docker image must be built locally from source.

Note for ARM/Apple Silicon Users: Docker will automatically build the image for your architecture.
...
(optional) Pull dependency images (PostgreSQL and Redis) by running the following command:

```bash
docker compose pull database redis
```

Start Cal.diy via docker compose (Most basic users, and for First Run). To run the complete stack, which includes a local Postgres database, Redis, Cal.diy web app, and Prisma Studio:
...
Re-build and start the Cal.diy stack
